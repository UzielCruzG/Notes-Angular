...
export class AddActivityCard implements OnInit {
  ...
  constructor(
    ...
    public dialog: MatDialogRef<AddActivityCard>,
    public route: ActivatedRoute
  ) {}

  ngOnInit() {
    this.form = new FormGroup({
      titleActivity: new FormControl(null, {
  ...
  onUpdateActivity(name: string, date: Date) {
    this.listsService.updateActivity(
      this.data.idList,
      this.data.idActivity,
      name,
      date
    );
    this.dialog.close();
